...
class MotChallenge2DBox(_BaseDataset):
    ...
    def __init__(self, config=None):
        ...
        # Bugfix: Correctly determine the GT folder path (self.gt_fol)
        if not self.config['SKIP_SPLIT_FOL']:
            split_fol = gt_set
        else:
            split_fol = ''

        # If a custom, absolute path is given for GT_FOLDER, use it directly.
        # Otherwise, use the default behavior of appending the split folder.
        if os.path.isabs(self.config['GT_FOLDER']):
    ...
    def _get_seq_info(self):
        ...
        if self.config["SEQ_INFO"]:
            ...
            # MODIFICATION: Instead of relying on a seqmap file, directly scan the GT folder for sequence directories.
            if not os.path.isdir(self.gt_fol):
                raise TrackEvalException('GT folder not found: ' + self.gt_fol)
            seq_dirs = [d for d in os.listdir(self.gt_fol) if os.path.isdir(os.path.join(self.gt_fol, d))]
            if not seq_dirs:
                raise TrackEvalException('No sequence directories found in ' + self.gt_fol)
            ...
            for seq in seq_dirs:
                ...
                ini_file = os.path.join(self.gt_fol, seq, 'seqinfo.ini')
                if not os.path.isfile(ini_file):
                    print('Warning: seqinfo.ini not found for sequence %s, skipping.' % seq)
                    continue
